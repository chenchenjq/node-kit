...
const problem = (code, message, retryable = false) => ({ code, message, retryable });
const fail = (code, message) => ({ ok: false, problem: problem(code, message) });
const fingerprint = (value) => JSON.stringify(value);
const validCommandId = (commandId) => commandId.trim().length > 0 && commandId.length <= 128;
...
const priceWarnings = (skus) => skus.some((sku) => sku.supplyPrice !== null && sku.suggestedRetailPrice !== null && compareDecimal(sku.supplyPrice, sku.suggestedRetailPrice) > 0)
    ? [{ code: "SUPPLY_PRICE_ABOVE_RETAIL", message: "供应价高于建议零售价" }] : [];
const clone = (value) => structuredClone(value);
const createDocument = (scopeKey, input) => ({
    scopeKey,
    spuId: input.spuId,
...
const validateStructure = (input) => {
    if (input.dimensions.length > MAX_DIMENSIONS)
        return problem("VALIDATION_FAILED", "销售规格维度不能超过五个");
    ...
    for (const dimension of input.dimensions) {
        ...
        if (!normalizedDimension || dimensions.has(normalizedDimension))
            return problem("VALIDATION_FAILED", "销售规格维度名称重复或为空");
        dimensions.add(normalizedDimension);
        if (dimension.values.length === 0 || dimension.values.length > MAX_VALUES_PER_DIMENSION)
            return problem("VALIDATION_FAILED", "每个维度必须有一到一百个规格值");
        const values = new Set();
        for (const value of dimension.values) {
            const normalizedValue = normalizeSpecificationLabel(value.label);
            if (!normalizedValue || values.has(normalizedValue))
                return problem("VALIDATION_FAILED", "同一维度的规格值重复或为空");
            values.add(normalizedValue);
        }
    }
    if (!calculatePotentialCombinations(input.dimensions.map((dimension) => dimension.values.length)).ok)
        return problem("VALIDATION_FAILED", "规格值的潜在组合不能超过一百个");
    if (input.selectedCombinations.length > MAX_COMBINATIONS)
        return problem("VALIDATION_FAILED", "实际销售组合不能超过一百个");
    if (input.dimensions.length === 0 && input.selectedCombinations.length !== 0)
        return problem("VALIDATION_FAILED", "无规格商品不能提交自定义组合");
    if (input.dimensions.length > 0 && input.selectedCombinations.length === 0)
        return problem("VALIDATION_FAILED", "多规格商品至少需要一个销售组合");
    return null;
};
const resolvePairs = (pairs, mappings, dimensions) => {
    ...
    if (resolved.some((pair) => pair.dimensionId === undefined || pair.valueId === undefined))
        return fail("VALIDATION_FAILED", "组合必须引用已知维度和值");
    if (resolved.length !== dimensions.length)
        return fail("VALIDATION_FAILED", "组合必须为每个销售规格维度选择一个值");
...
export const createSkuService = (options) => {
    const createId = options.createId ?? (() => crypto.randomUUID());
    const scopeFor = async (context) => {
        const scope = await options.resolveScope(context);
        return scope.trim() === "" ? null : scope;
    };
    const allowed = async (context, action, spuId) => {
    ...
    const inventoryAuthorityFor = async (context, scopeKey, bind = true) => {
        const authority = await (options.resolveInventoryAuthority?.(context, scopeKey) ?? Promise.resolve({ kind: "local-read-write", authorityKey: "sku-kit-local" }));
        if (authority.authorityKey.trim() === "")
            return fail("VALIDATION_FAILED", "库存权威键不能为空");
    ...
    const preview = (existing, input) => {
        ...
        const dimensions = input.dimensions.map((dimension) => {
            ...
            if (dimension.draftKey !== undefined)
                mappings[dimension.draftKey] = id;
            return { id, label: dimension.label, normalizedLabel: normalizeSpecificationLabel(dimension.label), sort: dimension.sort, archived: false, values: dimension.values.map((value) => {
                    ...
                    if (value.draftKey !== undefined)
                        mappings[value.draftKey] = valueId;
        ...
        for (const candidate of proposed) {
            ...
            if (keys.has(key))
                return fail("COMBINATION_CONFLICT", "同一 SPU 不允许重复规格组合");
            keys.add(key);
        ...
        for (const sku of existing?.skus ?? [])
            if (!sku.archived && !keys.has(sku.combinationKey))
                combinations.push({ combinationKey: sku.combinationKey, kind: "archivable", skuId: sku.id, skuCode: sku.skuCode });
    ...
    return {
        async previewConfiguration(context, input) {
            ...
            if (scopeKey === null)
                return fail("SCOPE_MISMATCH", "可信作用域不能为空");
            ...
            if (existing !== null && existing.registeredSpuCode !== input.spuCode)
                return fail("SPU_CODE_MISMATCH", "SPU 编码已冻结，不能修改");
        ...
        async saveConfiguration(context, input) {
            ...
            if (!authorization.ok)
                return authorization;
            if (!validCommandId(input.commandId))
                return fail("VALIDATION_FAILED", "commandId 必须是 1 至 128 个非空白字符");
            const inventoryAuthority = await inventoryAuthorityFor(context, scopeKey);
            if (!inventoryAuthority.ok)
                return inventoryAuthority;
            const code = validateRegisteredSpuCode(input.spuCode);
            if (!code.ok)
                return code;
            const existingCode = await options.store.findByRegisteredSpuCode?.(scopeKey, code.value);
            if (existingCode !== undefined && existingCode !== null && existingCode.spuId !== input.spuId)
                return fail("SPU_CODE_CONFLICT", "同一作用域内 SPU 编码必须唯一");
            ...
            return options.store.transact(scopeKey, input.spuId, async (existing) => {
                ...
                if (document.registeredSpuCode !== input.spuCode)
                    return { document, result: fail("SPU_CODE_MISMATCH", "SPU 编码已冻结，不能修改") };
                ...
                if (receipt !== undefined) {
                    return {
                        ...
                        result: receipt.fingerprint === requestFingerprint
                            ? { ok: true, value: clone(receipt.result) }
                ...
                const dimensions = input.dimensions.map((dimension) => {
                    ...
                    if (dimension.draftKey !== undefined)
                        mappings[dimension.draftKey] = dimensionId;
                    ...
                    if (previous !== undefined) {
                        for (const oldValue of previous.values)
                            if (!values.some((value) => value.id === oldValue.id))
                ...
                for (const oldDimension of document.dimensions)
                    if (!dimensions.some((dimension) => dimension.id === oldDimension.id))
                        dimensions.push({ ...oldDimension, archived: true, values: oldDimension.values.map((value) => ({ ...value, archived: true })) });
                ...
                for (const combination of combinations) {
                    ...
                    if (!resolvedPairs.ok)
                        return { document, result: resolvedPairs };
                    const key = buildCombinationKey(resolvedPairs.value);
                    if (seen.has(key))
                        return { document, result: fail("COMBINATION_CONFLICT", "同一 SPU 不允许重复规格组合") };
                    seen.add(key);
                    ...
                    if (oldSku !== undefined && oldSku.archived) {
                        if (!(input.restoreArchivedSkuIds ?? []).includes(oldSku.id))
                            return { document, result: fail("RESTORE_REQUIRED", "恢复历史组合必须显式指定原 SKU") };
                        skus.push({ ...oldSku, pairs: resolvedPairs.value, archived: false, status: "disabled", configVersion: oldSku.configVersion + 1n });
                        continue;
                    }
                    if (document.nextSequence >= 100)
                        return { document, result: fail("CAPACITY_EXHAUSTED", "SKU 编码容量已耗尽") };
                    const formatted = formatSkuCode(document.registeredSpuCode, document.nextSequence);
                    if (!formatted.ok)
                        return { document, result: formatted };
                ...
                for (const oldSku of document.skus) {
                    if (!oldSku.archived && !skus.some((sku) => sku.id === oldSku.id))
                        skus.push({ ...oldSku, archived: true, status: "archived", configVersion: oldSku.configVersion + 1n });
                }
        ...
        async patchSkuConfigurations(context, input) {
            const scopeKey = await scopeFor(context);
            if (scopeKey === null)
                return fail("SCOPE_MISMATCH", "可信作用域不能为空");
            const configurationAuthorization = await allowed(context, "configuration.write", input.spuId);
            if (!configurationAuthorization.ok)
                return configurationAuthorization;
            if (!validCommandId(input.commandId))
                return fail("VALIDATION_FAILED", "commandId 必须是 1 至 128 个非空白字符");
            const inventoryAuthority = await inventoryAuthorityFor(context, scopeKey);
            if (!inventoryAuthority.ok)
            ...
            if (input.patches.some((patch) => patch.supplyPrice !== undefined)) {
                ...
                if (!supplyAuthorization.ok)
                    return supplyAuthorization;
            }
            if (input.patches.length === 0)
                return fail("VALIDATION_FAILED", "至少需要一个 SKU 配置补丁");
            const requestFingerprint = fingerprint(input);
            return options.store.transact(scopeKey, input.spuId, (existing) => {
                if (existing === null)
                    return { result: fail("NOT_FOUND", "未找到 SKU 配置") };
                const document = clone(existing);
                const receipt = document.commands.find((candidate) => candidate.commandId === input.commandId);
                if (receipt !== undefined) {
                    const replay = clone(receipt.result);
                ...
                for (const patch of input.patches) {
                    if (ids.has(patch.skuId))
                        return { document, result: fail("VALIDATION_FAILED", "同一 SKU 只能出现一次") };
                    ids.add(patch.skuId);
                    const sku = document.skus.find((candidate) => candidate.id === patch.skuId);
                    if (sku === undefined || sku.archived)
                        return { document, result: fail("NOT_FOUND", "未找到可配置 SKU") };
                    if (sku.configVersion.toString() !== patch.expectedConfigVersion)
                        return { document, result: fail("VERSION_CONFLICT", "SKU 配置已被其他编辑修改") };
                    if (patch.suggestedRetailPrice !== undefined && patch.suggestedRetailPrice !== null) {
                        ...
                        if (!price.ok)
                            return { document, result: price };
                        sku.suggestedRetailPrice = price.value;
                    }
                    else if (patch.suggestedRetailPrice === null)
                        sku.suggestedRetailPrice = null;
                    ...
                    else if (patch.supplyPrice === null)
                        sku.supplyPrice = null;
                    if (patch.image !== undefined)
                        sku.image = patch.image;
                    if (patch.status !== undefined)
                        sku.status = patch.status;
        ...
        async copySkuConfiguration(context, input) {
            const scopeKey = await scopeFor(context);
            if (scopeKey === null)
                return fail("SCOPE_MISMATCH", "可信作用域不能为空");
            ...
            if (!validCommandId(input.commandId))
                return fail("VALIDATION_FAILED", "commandId 必须是 1 至 128 个非空白字符");
            if (input.fields.includes("supplyPrice")) {
                const supply = await allowed(context, "supply-price.write", input.spuId);
                if (!supply.ok)
                    return supply;
            }
            if (input.fields.length === 0 || input.targetSkuIds.length === 0 || new Set(input.targetSkuIds).size !== input.targetSkuIds.length)
                return fail("VALIDATION_FAILED", "复制必须指定不重复的字段和目标 SKU");
            const authority = await inventoryAuthorityFor(context, scopeKey);
            if (!authority.ok)
                return authority;
            const requestFingerprint = fingerprint(input);
            return options.store.transact(scopeKey, input.spuId, (existing) => {
                if (existing === null)
                    return { result: fail("NOT_FOUND", "未找到 SKU 配置") };
                const document = clone(existing);
                const receipt = document.commands.find((item) => item.commandId === input.commandId);
                if (receipt !== undefined)
                    return { document, result: receipt.fingerprint === requestFingerprint ? { ok: true, value: clone(receipt.result) } : fail("IDEMPOTENCY_CONFLICT", "相同 commandId 使用了不同载荷") };
                const source = document.skus.find((sku) => sku.id === input.sourceSkuId && !sku.archived);
                if (source === undefined)
                    return { document, result: fail("NOT_FOUND", "未找到来源 SKU") };
                const targets = input.targetSkuIds.map((id) => document.skus.find((sku) => sku.id === id && !sku.archived));
                if (targets.some((target) => target === undefined) || input.targetSkuIds.includes(source.id))
                    return { document, result: fail("VALIDATION_FAILED", "复制目标必须是其他有效 SKU") };
                const copied = [];
                for (const target of targets) {
                    for (const field of input.fields) {
                        if (field === "suggestedRetailPrice" && (input.mode === "overwrite" || target.suggestedRetailPrice === null))
                            target.suggestedRetailPrice = source.suggestedRetailPrice;
                        if (field === "supplyPrice" && (input.mode === "overwrite" || target.supplyPrice === null))
                            target.supplyPrice = source.supplyPrice;
                        if (field === "image" && (input.mode === "overwrite" || target.image === null))
        ...
        async getManagementConfiguration(context, input) {
            const scopeKey = await scopeFor(context);
            if (scopeKey === null)
                return fail("SCOPE_MISMATCH", "可信作用域不能为空");
            ...
            if (document === null)
                return fail("NOT_FOUND", "未找到 SKU 配置");
        ...
        async getHistoricalConfiguration(context, input) {
            const scopeKey = await scopeFor(context);
            if (scopeKey === null)
                return fail("SCOPE_MISMATCH", "可信作用域不能为空");
        ...
        async getConsumerSelection(context, input) {
            const scopeKey = await scopeFor(context);
            if (scopeKey === null)
                return fail("SCOPE_MISMATCH", "可信作用域不能为空");
            ...
            for (const dimension of document.dimensions)
                if (!dimension.archived)
                    for (const value of dimension.values)
                        if (!value.archived)
                            valueOwners.set(value.id, dimension.id);
            const chosenDimensions = new Set();
            for (const valueId of input.selectedValueIds) {
                const dimensionId = valueOwners.get(valueId);
                if (dimensionId === undefined || chosenDimensions.has(dimensionId))
                    return fail("VALIDATION_FAILED", "每个规格维度只能选择一个有效规格值");
            ...
            const known = priceCandidates.map((sku) => projections.get(sku.id)?.salePrice).filter((price) => price !== null && price !== undefined).sort(compareDecimal);
            const range = known.length === 0 ? { status: "unknown", min: null, max: null } : { status: known.length === priceCandidates.length ? "complete" : "partial", min: known[0] ?? null, max: known.at(-1) ?? null };
            const dimensions = document.dimensions.filter((dimension) => !dimension.archived).map((dimension) => ({ dimensionId: dimension.id, label: dimension.label, options: dimension.values.filter((value) => !value.archived).map((value) => {
                    ...
                    return { valueId: value.id, label: value.label, state: selectedHere ? "selected" : projection !== undefined ? "selectable" : valueCandidates.some((sku) => projections.get(sku.id)?.availability === "out-of-stock") ? "out-of-stock" : valueCandidates.length === 0 ? "impossible" : "unavailable" };
                }) }));
        ...
        async getInventory(context, input) {
            ...
            if (!authority.ok)
                return authority;
            const document = await options.store.read(scopeKey, input.spuId);
            if (document === null || !document.skus.some((sku) => sku.id === input.skuId && !sku.archived))
                return fail("NOT_FOUND", "未找到可读取的 SKU 库存");
            ...
            if (document === null || inventory === undefined || !document.skus.some((sku) => sku.id === input.skuId && !sku.archived))
                return fail("NOT_FOUND", "未找到可读取的 SKU 库存");
        ...
        async setLocalInventory(context, input) {
            const scopeKey = await scopeFor(context);
            if (scopeKey === null)
                return fail("SCOPE_MISMATCH", "可信作用域不能为空");
            ...
            if (authority.kind !== "local-read-write")
                return fail(authority.kind === "external-read-only" ? "INVENTORY_READ_ONLY" : "ADAPTER_FAILURE", "当前作用域不由本地库存权威管理");
            if (!Number.isInteger(input.quantity) || input.quantity < 0)
                return fail("VALIDATION_FAILED", "库存必须是非负整数");
            const requestFingerprint = fingerprint(input);
            return options.store.transact(scopeKey, input.spuId, (existing) => {
                if (existing === null)
                    return { result: fail("NOT_FOUND", "未找到 SKU 配置") };
                ...
                const inventory = document.inventories[input.skuId];
                if (sku === undefined || inventory === undefined)
                    return { document, result: fail("NOT_FOUND", "未找到可配置 SKU 库存") };
                if (inventory.version.toString() !== input.expectedInventoryVersion)
                    return { document, result: fail("VERSION_CONFLICT", "库存已被其他编辑修改") };
        ...
        async setExternalInventory(context, input) {
            const scopeKey = await scopeFor(context);
            if (scopeKey === null)
                return fail("SCOPE_MISMATCH", "可信作用域不能为空");
            const authorization = await allowed(context, "inventory.write", input.spuId);
            ...
            if (!resolved.ok)
                return resolved;
            if (resolved.value.kind === "external-read-only")
                return fail("INVENTORY_READ_ONLY", "外部库存权威为只读");
            if (resolved.value.kind !== "external-read-write")
                return fail("VALIDATION_FAILED", "当前作用域不由外部库存权威管理");
            if (options.setExternalInventory === undefined)
                return fail("ADAPTER_FAILURE", "外部库存写入适配器不可用");
