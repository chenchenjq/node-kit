import type { CombinationPair, Result, SkuProblem } from "./types.js";
...
const validationFailure = (message: string, details?: Readonly<Record<string, unknown>>): SkuProblem => ({
...
export const normalizeSpecificationLabel = (label: string): string =>
  label.normalize("NFKC").trim().replace(/\s+/gu, " ").toLowerCase();

export const buildCombinationKey = (pairs: readonly CombinationPair[]): string => {
  const sorted = [...pairs]
    .map(({ dimensionId, valueId }) => [dimensionId, valueId] as const)
    .sort(([left], [right]) => left.localeCompare(right));
  return `v1:${JSON.stringify(sorted)}`;
};

export const calculatePotentialCombinations = (
  valueCounts: readonly number[],
...
export const validateCnyAmount = (amount: string): Result<string> => {
...
export const validateRegisteredSpuCode = (value: string): Result<string> => {
...
export const formatSkuCode = (registeredSpuCode: string, sequence: number): Result<string> => {
