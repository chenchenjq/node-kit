import type { CommandReceipt, InventoryAuthority, PersistedDimension, PersistedSku, ProductDocument, SkuStore } from "../server/contracts.js";
...
export interface PgQueryClient {
  query<Row extends Record<string, unknown> = Record<string, unknown>>(sql: string, values?: readonly unknown[]): Promise<{ rows: Row[] }>;
  release(): void;
}
...
interface ProductRow extends Record<string, unknown> {
  scope_key: string; spu_id: string; registered_spu_code: string; structure_version: string; next_sequence: number;
}
...
const tableNames = (schemaName: string) => {
...
const readDocument = async (client: PgTransactionClient, schemaName: string, scopeKey: string, spuId: string, lock = false): Promise<ProductDocument | null> => {
  const t = tableNames(schemaName);
  const product = await client.query<ProductRow>(`SELECT scope_key, spu_id, registered_spu_code, structure_version, next_sequence FROM ${t.product} WHERE scope_key = $1 AND spu_id = $2${lock ? " FOR UPDATE" : ""}`, [scopeKey, spuId]);
  ...
  const valuesByDimension = new Map<string, ProductDocument["dimensions"][number]["values"]>();
  ...
  const persistedDimensions: PersistedDimension[] = dimensions.rows.map((row) => ({
  ...
  const pairsBySku = new Map<string, PersistedSku["pairs"]>();
  ...
  const persistedSkus: PersistedSku[] = skus.rows.map((row) => ({
    id: String(row.id), skuCode: String(row.sku_code), sequence: Number(row.sequence), combinationKey: String(row.combination_key), pairs: pairsBySku.get(String(row.id)) ?? [], status: String(row.status) as PersistedSku["status"], suggestedRetailPrice: nullable(row.suggested_retail_price), supplyPrice: nullable(row.supply_price), image: row.image_adapter === null ? null : { adapter: String(row.image_adapter), value: String(row.image_value) }, configVersion: bigint(row.config_version), archived: row.archived_at !== null,
  }));
  const inventory: ProductDocument["inventories"] = {};
  for (const row of inventories.rows) inventory[String(row.sku_id)] = { quantity: Number(row.quantity), version: bigint(row.version) };
  const commands: CommandReceipt[] = receipts.rows.map((row) => ({ commandId: String(row.command_id), fingerprint: String(row.fingerprint), result: row.result }));
...
const persistDocument = async (client: PgTransactionClient, schemaName: string, document: ProductDocument): Promise<void> => {
  const t = tableNames(schemaName);
  ...
  for (const dimension of document.dimensions) for (const value of dimension.values) {
  ...
  for (const sku of document.skus) for (const pair of sku.pairs) await client.query(`INSERT INTO ${t.selection} (sku_id, scope_key, spu_id, dimension_id, value_id) VALUES ($1,$2,$3,$4,$5)`, [sku.id, document.scopeKey, document.spuId, pair.dimensionId, pair.valueId]);
  for (const [skuId, entry] of Object.entries(document.inventories)) await client.query(`INSERT INTO ${t.inventory} (sku_id, quantity, version) VALUES ($1,$2,$3) ON CONFLICT (sku_id) DO UPDATE SET quantity = EXCLUDED.quantity, version = EXCLUDED.version, updated_at = now()`, [skuId, entry.quantity, entry.version.toString()]);
 ...
 */
export const createPostgresSkuStore = (pool: PgPool, options: { schemaName?: string } = {}): SkuStore => {
  ...
  return {
    async transact(scopeKey, spuId, callback) {
      ...
      try {
        ...
        if (outcome.document !== undefined) await persistDocument(client, schemaName, outcome.document);
    ...
    async read(scopeKey, spuId) {
      const client = await pool.connect();
      try { return await readDocument(client, schemaName, scopeKey, spuId); }
      finally { client.release(); }
    },
    async findByRegisteredSpuCode(scopeKey, registeredSpuCode) {
      const client = await pool.connect();
      try {
        const row = await client.query<ProductRow>(`SELECT scope_key, spu_id FROM ${tableNames(schemaName).product} WHERE scope_key = $1 AND registered_spu_code = $2`, [scopeKey, registeredSpuCode]);
    ...
    async ensureInventoryAuthority(scopeKey, authority): Promise<Result<InventoryAuthority>> {
      const client = await pool.connect(); const t = tableNames(schemaName);
      try {
        ...
        if (result.rows.length === 0) return { ok: false, problem: { code: "INVENTORY_AUTHORITY_MISMATCH", message: "库存权威已绑定，不能静默修改", retryable: false } };
 ...
 */
export const createPostgresSkuStoreForTransaction = (client: PgTransactionClient, options: { schemaName?: string } = {}): SkuStore => {
  const schemaName = options.schemaName ?? "public";
  validateSkuSchemaName(schemaName);
  return {
    async transact(scopeKey, spuId, callback) {
