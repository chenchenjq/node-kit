...
export interface AuthorizationAllowed { ok: true }
export interface AuthorizationDenied { ok: false; problem?: SkuProblem }
...
export interface SpuRefInput { spuId: string }
...
export interface ProductDocument {
  ...
  skus: PersistedSku[];
  inventories: Record<string, { quantity: number; version: bigint }>;
...
export type InventoryReadDto =
  | { state: "known"; skuId: string; quantity: number; inventoryVersion: string }
  | { state: "unknown"; skuId: string; reason: string };
...
export interface SkuStore {
  transact<T>(scopeKey: string, spuId: string, callback: (current: ProductDocument | null) => Promise<{ document?: ProductDocument; result: T }> | { document?: ProductDocument; result: T }): Promise<T>;
...
export interface ManagementConfiguration {
  ...
  skus: readonly (Omit<PersistedSku, "supplyPrice"> & { supplyPrice?: string | null })[];
}
...
export interface ConsumerSelectionDto {
  currency: "CNY";
  dimensions: readonly { dimensionId: string; label: string; options: readonly ConsumerOption[] }[];
  selectedSku: { skuId: string; skuCode: string; salePrice: string | null; availability: string } | null;
  image: SkuImageReference | null;
  priceRange: { status: "complete" | "partial" | "unknown"; min: string | null; max: string | null };
}
...
export interface SkuService<RequestContext> {
  ...
  getInventory(context: RequestContext, input: SpuRefInput & { skuId: string }): Promise<Result<InventoryReadDto>>;
